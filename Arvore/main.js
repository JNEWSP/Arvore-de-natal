...
// Adicione um novo import para sons
const soundClick = new Audio('path/to/click-sound.mp3'); // Substitua pelo caminho do seu arquivo de som
const backgroundMusic = new Audio('path/to/background-music.mp3'); // Substitua pelo caminho da sua música
...
const stripes = [];
const balls = [];
const snowflakes = []; // Array para flocos de neve
...
window.addEventListener("click", e => {
    ...
    if (intersects.length > 0) {
        soundClick.play(); // Toca o som ao clicar
        addBall(intersects[0].point, balls.length - 1, performance.now());
        createParticles(intersects[0].point); // Cria partículas ao clicar
    }
});
...
function createSnowflakes() {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        snowflake.position.x = Math.random() * 10 - 5; // Posição aleatória em x
        snowflake.position.y = Math.random() * 10 + 5; // Começa acima da cena
        snowflake.position.z = Math.random() * 10 - 5; // Posição aleatória em z
...
function animateSnowflakes() {
    snowflakes.forEach(snowflake => {
        snowflake.position.y -= 0.02; // Faz a neve cair
        if (snowflake.position.y < -5) {
            snowflake.position.y = Math.random() * 10 + 5; // Reseta a posição quando sai da tela
            snowflake.position.x = Math.random() * 10 - 5; // Nova posição aleatória em x
            snowflake.position.z = Math.random() * 10 - 5; // Nova posição aleatória em z
        }
    });
}
...
function addBall(pos, bIdx, clickTime) {
    const material = new THREE.MeshStandardMaterial({
        color: new THREE.Color().setHSL(Math.random(), 1, 0.5), // Cor aleatória
        roughness: .2,
        metalness: .5,
    });
    ...
    ball.scale.set(0, 0, 0); // Começa com escala zero
    ball.position.copy(pos);

    // Animação de "crescimento"
    const growthAnimation = setInterval(() => {
        ball.scale.x += 0.05; // Cresce
        ball.scale.y += 0.05;
        ball.scale.z += 0.05;
        if (ball.scale.x >= scale) {
            clearInterval(growthAnimation); // Para a animação quando atinge o tamanho
        }
    }, 50);
...
function render(time) {
    ...
    const radius = 0.8; // Ajuste o raio para que as bolas fiquem mais próximas da árvore
    balls.forEach((b, bIdx) => {
        const aT = .001 * (time - b.clickTime);
        const angle = (bIdx + t) * 0.5; // Aumenta o ângulo para fazer a bola circular

        // Cálculo da posição em um círculo
        const posY = 0; // Altura fixa para as bolas
        const posX = radius * Math.cos(angle); // Cálculo da posição X
        const posZ = radius * Math.sin(angle); // Cálculo da posição Z

        const posTarget = new THREE.Vector3(posX, posY, posZ); // Mantém a altura constante
